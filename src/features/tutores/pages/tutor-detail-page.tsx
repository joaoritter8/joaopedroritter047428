              ...
              {tutor.pets?.length ? (
                <div className="grid gap-2">
                  {tutor.pets.map((p) => (
                    <div
                      key={p.id}
                      className="flex flex-col gap-2 rounded-2xl border border-zinc-200 bg-white p-3 sm:flex-row sm:items-center sm:justify-between"
                    >
                      <div className="min-w-0">
                        <div className="truncate font-medium text-zinc-900">{p.nome}</div>
                        <div className="text-xs text-zinc-600">
                          {p.raca ? `Raça: ${p.raca}` : 'Espécie/Raça: —'} •{' '}
                          {typeof p.idade === 'number' ? `Idade: ${p.idade}` : 'Idade: —'}
                        </div>
                      </div>
                  ...
                  {petsQ.isLoading ? (
                    ...
                    <div className="grid gap-2">
                      {petsQ.data.content.slice(0, 5).map((p) => (
                        <div
                          key={p.id}
                          className="flex items-center justify-between gap-2 rounded-xl border border-zinc-200 bg-white p-3"
                        >
                          <div className="min-w-0">
                          ...
                          <Button onClick={() => linkM.mutate(p.id)} disabled={linkM.isPending}>
                            <Link2 size={16} />
                            Vincular
                          </Button>
                        </div>
