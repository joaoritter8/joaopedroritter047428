  ...
  return (
    <div className="grid gap-4">
      <Card>
        <CardContent className="flex flex-col gap-3 p-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <p className="text-sm text-zinc-600">Listagem paginada de tutores com busca por nome.</p>
          </div>
          ...
          <div className="w-full sm:max-w-sm">
            ...
            <div className="relative">
              ...
              <Input className="pl-9" value={draft} onChange={(e) => setDraft(e.target.value)} placeholder="ex: Maria" />
            </div>
          </div>
        </CardContent>
      </Card>
      ...
      {isLoading ? (
        ...
        <div className="rounded-2xl border border-red-200 bg-white p-6 text-sm text-red-700">Erro ao carregar: {(error as Error).message}</div>
      ) : data ? (
        <>
